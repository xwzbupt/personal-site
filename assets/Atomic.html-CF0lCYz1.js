...
<span class="line"><span>}</span></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>接下来，我们重点讲解一下AtomicInteger提供的4类核心原子操作：CAS、增加、自增、自减。</p><p><strong>1）CAS函数</strong></p><p>AtomicInteger中的compareAndSet()是标准的CAS函数。如下代码所示，如果value值等于入参expect值，那么就将value值更新为入参update值，并返回true。compareAndSet()函数底层调用Unsafe类的CAS方法compareAndSwapInt()来实现，此方法在上一节中已经详细讲解，这里就不再赘述了。</p><div class="language- line-numbers-mode" data-highlighter="shiki" data-ext="" style="--shiki-light:#383A42;--shiki-dark:#abb2bf;--shiki-light-bg:#FAFAFA;--shiki-dark-bg:#282c34;"><pre class="shiki shiki-themes one-light one-dark-pro vp-code"><code class="language-"><span class="line"><span>public final boolean compareAndSet(int expect, int update) {</span></span>
<span class="line"><span>    return unsafe.compareAndSwapInt(this, valueOffset, expect, update);</span></span>
<span class="line"><span>}</span></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><strong>2）增加函数</strong></p><p>增加函数有两个。从命名上，我们也可以大概猜出两者的不同之处。其中，getAndAdd()函数先获取value值，再更新value，函数返回更新前的旧值，addAndGet()函数先更新value，再获取value值，函数返回更新之后的新值。</p><div class="language- line-numbers-mode" data-highlighter="shiki" data-ext="" style="--shiki-light:#383A42;--shiki-dark:#abb2bf;--shiki-light-bg:#FAFAFA;--shiki-dark-bg:#282c34;"><pre class="shiki shiki-themes one-light one-dark-pro vp-code"><code class="language-"><span class="line"><span>public final int getAndAdd(int delta) {</span></span>
...
<span class="line"><span>}</span></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>两个函数的代码实现非常相似，addAndGet()函数只不过是在getAndAdd()返回值的基础之上，加了一个delta再返回而已。因此，我们重点看下getAndAdd()函数。getAndAdd()函数调用Unsafe类的getAndAddInt()方法来实现。getAndAddInt()方法的代码实现如下所示。</p><div class="language- line-numbers-mode" data-highlighter="shiki" data-ext="" style="--shiki-light:#383A42;--shiki-dark:#abb2bf;--shiki-light-bg:#FAFAFA;--shiki-dark-bg:#282c34;"><pre class="shiki shiki-themes one-light one-dark-pro vp-code"><code class="language-"><span class="line"><span>public final int getAndAddInt(Object o, long offset, int delta) {</span></span>
...
<span class="line"><span>}</span></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>getAndAddInt()并非native方法，而是直接由Java代码实现，其底层调用Unsafe类的CAS方法compareAndSwapInt()来实现。在多线程竞争执行时，CAS有可能执行失败，因此，getAndAddInt()采用自旋重复执行CAS，直到成功为止。这样就可以保证getAndAddInt()总是可以将value值增加delta。</p><p><strong>3）自增函数</strong></p><p>自增函数也有两个。用法跟增加函数类似，getAndIncrement()函数返回自增之前的旧值，incrementAndGet()函数返回自增之后的新值。底层实现原理也跟增加函数类似，只需要将增加函数中的delta设置为1即可。</p><div class="language- line-numbers-mode" data-highlighter="shiki" data-ext="" style="--shiki-light:#383A42;--shiki-dark:#abb2bf;--shiki-light-bg:#FAFAFA;--shiki-dark-bg:#282c34;"><pre class="shiki shiki-themes one-light one-dark-pro vp-code"><code class="language-"><span class="line"><span>public final int getAndIncrement() { </span></span>
...
<span class="line"><span>}</span></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><strong>4）自减函数</strong></p><p>自减函数也有两个。用法跟自增函数类似，getAndDecrement()函数返回自减之前的旧值，decrementAndGet()函数返回自减之后的新值。底层实现原理跟增加函数类似，只需要将增加函数中的delta设置为-1即可。</p><div class="language- line-numbers-mode" data-highlighter="shiki" data-ext="" style="--shiki-light:#383A42;--shiki-dark:#abb2bf;--shiki-light-bg:#FAFAFA;--shiki-dark-bg:#282c34;"><pre class="shiki shiki-themes one-light one-dark-pro vp-code"><code class="language-"><span class="line"><span>public final int getAndDecrement() {</span></span>
<span class="line"><span>    return unsafe.getAndAddInt(this, valueOffset, -1);</span></span>
<span class="line"><span>}</span></span>
<span class="line"><span>public final int decrementAndGet() {</span></span>
<span class="line"><span>    return unsafe.getAndAddInt(this, valueOffset, -1) - 1;</span></span>
<span class="line"><span>}</span></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>从功能上，AtomicInteger提供的增加函数，完全可以替代自增函数和自减函数。AtomicInteger类提供冗余的自增函数和自减函数，完全是为了方便程序员使用。</p><h2 id="三、引用类型原子类" tabindex="-1"><a class="header-anchor" href="#三、引用类型原子类"><span><strong>三、引用类型原子类</strong></span></a></h2><p>基本类型原子类提供了操作基本类型数据的原子函数，同理，引用类型原子类提供了操作引用类型数据的原子函数。JUC提供了3个引用类型原子类，它们分别是：AtomicReference、AtomicStampedReference、AtomicMarkableReference。接下来，我们一一详细讲解这3个引用类型原子类的用法和实现原理。</p><p><strong>1）AtomicReference</strong></p><p>AtomicReference类的部分源码如下所示。基本类型原子类主要依赖Unsafe类中的compareAndSwapInt()和compareAndSwapLong()这两个CAS方法来实现。引用类型原子类主要依赖Unsafe类中的compareAndSwapObject()这个CAS方法来实现。AtomicReference的实现方式跟AtomicInteger类似，这里就不再赘述了。</p><div class="language- line-numbers-mode" data-highlighter="shiki" data-ext="" style="--shiki-light:#383A42;--shiki-dark:#abb2bf;--shiki-light-bg:#FAFAFA;--shiki-dark-bg:#282c34;"><pre class="shiki shiki-themes one-light one-dark-pro vp-code"><code class="language-"><span class="line"><span>public class AtomicReference&lt;V&gt; {</span></span>
...
<span class="line"><span>    }</span></span>
<span class="line"><span>}</span></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>AtomicReference类的使用方式，如下示例代码所示。</p><div class="language- line-numbers-mode" data-highlighter="shiki" data-ext="" style="--shiki-light:#383A42;--shiki-dark:#abb2bf;--shiki-light-bg:#FAFAFA;--shiki-dark-bg:#282c34;"><pre class="shiki shiki-themes one-light one-dark-pro vp-code"><code class="language-"><span class="line"><span>public class DemoLock {</span></span>
...
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><strong>2）AtomicStampedReference</strong></p><p>相对于AtomicReference，AtomicStampedReference增加了版本戳，主要是用来解决CAS的ABA问题。什么是CAS的ABA问题呢？我们举例解释一下。如下代码所示，addAtHead()函数往链表头部添加节点，removeAtHead()函数从链表头部移除节点。你觉得下面的代码是否是线程安全的呢？</p><div class="language- line-numbers-mode" data-highlighter="shiki" data-ext="" style="--shiki-light:#383A42;--shiki-dark:#abb2bf;--shiki-light-bg:#FAFAFA;--shiki-dark-bg:#282c34;"><pre class="shiki shiki-themes one-light one-dark-pro vp-code"><code class="language-"><span class="line"><span>public class Node {</span></span>
...
<span class="line"><span>    }</span></span>
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>不过，以上代码实现仍然存在问题。如下图所示。线程1执行了一次removeAtHead()函数，线程2执行了两次removeAtHead()函数和一次addAtHead()函数，因此，链表中的节点个数最终应该是1，但实际的运行结果却是2。这是为什么呢？</p><figure><img src="http://wechatapppro-1252524126.file.myqcloud.com/appnvnpyonz2273/image/ueditor/26761400_1659454052.png" alt="img" tabindex="0" loading="lazy"><figcaption>img</figcaption></figure><p>实际上，引起执行结果出错的原因，就是CAS的ABA问题。线程1执行removeAtHead()函数，设置oldHead=A、nextNode=B，在执行CAS之前，线程2将head从A变为B、C，最后又变为A。尽管线程2执行完之后，head仍为A，但链表的整体结构发生了变化。随后，当线程1执行CAS时，检查当前的head跟oldHead相等，仍然是A，错以为期间没有其他线程执行addAtHead()和removeAtHead()函数，于是，成功执行CAS。</p><p>为了解决以上ABA问题，我们使用AtomicStampedReference对LinkedList进行改造，如下代码所示。</p><div class="language- line-numbers-mode" data-highlighter="shiki" data-ext="" style="--shiki-light:#383A42;--shiki-dark:#abb2bf;--shiki-light-bg:#FAFAFA;--shiki-dark-bg:#282c34;"><pre class="shiki shiki-themes one-light one-dark-pro vp-code"><code class="language-"><span class="line"><span>public class LinkedList {</span></span>
...
<span class="line"><span>    }</span></span>
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>AtomicStampedReference的部分源码如下所示。相对于AtomicReference，AtomicStampedReference增加了一个int类型stamp版本戳，它将stamp和引用对象reference（例如head）封装成一个新的Pair对象，在Pair对象上执行CAS。即便引用对象存在ABA问题，但是stamp总是在增加，stamp不会存在ABA问题，因此，两者组合而成的Pair对象，也不就不存在ABA问题了。</p><div class="language- line-numbers-mode" data-highlighter="shiki" data-ext="" style="--shiki-light:#383A42;--shiki-dark:#abb2bf;--shiki-light-bg:#FAFAFA;--shiki-dark-bg:#282c34;"><pre class="shiki shiki-themes one-light one-dark-pro vp-code"><code class="language-"><span class="line"><span>public class AtomicStampedReference&lt;V&gt; {</span></span>
...
<span class="line"><span>}</span></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="五、对象属性原子类" tabindex="-1"><a class="header-anchor" href="#五、对象属性原子类"><span><strong>五、对象属性原子类</strong></span></a></h2><p>JUC提供了3个对象属性原子类，它们分别是：AtomicIntegerFiledUpdater、AtomicLongFieldUpdater、AtomicReferenceFiledUpdater。如果某个类中的属性没有提供合适的原子操作，那么我们可以使用对象属性原子类来对其进行原子操作。不过，允许这样做的前提是：属性必须是public的。</p><p>因为对象属性原子类很少用到，所以，我们仅仅举例简单介绍一下，不做深入分析。</p><div class="language- line-numbers-mode" data-highlighter="shiki" data-ext="" style="--shiki-light:#383A42;--shiki-dark:#abb2bf;--shiki-light-bg:#FAFAFA;--shiki-dark-bg:#282c34;"><pre class="shiki shiki-themes one-light one-dark-pro vp-code"><code class="language-"><span class="line"><span>public class Updater {</span></span>
...
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="六、课后思考题" tabindex="-1"><a class="header-anchor" href="#六、课后思考题"><span><strong>六、课后思考题</strong></span></a></h2><p>对于AtomicInteger类中的getAndAdd()函数，如果我们不使用CAS来实现，那么，如下代码实现方式，是否是线程安全的？为什么呢？</p><div class="language- line-numbers-mode" data-highlighter="shiki" data-ext="" style="--shiki-light:#383A42;--shiki-dark:#abb2bf;--shiki-light-bg:#FAFAFA;--shiki-dark-bg:#282c34;"><pre class="shiki shiki-themes one-light one-dark-pro vp-code"><code class="language-"><span class="line"><span>public final int getAndAdd(int delta) {</span></span>
<span class="line"><span>    int oldValue = value;</span></span>
<span class="line"><span>    value += delta;</span></span>
<span class="line"><span>    return oldValue;</span></span>
<span class="line"><span>}</span></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div>`,66)]))}const r=s(l,[["render",p]]),v=JSON.parse('{"path":"/language/Java/multithreading/lockless/Atomic.html","title":"原子类","lang":"zh-CN","frontmatter":{"category":"Java","feed":false,"seo":false,"head":[]},"git":{"createdTime":1750518217000,"updatedTime":1750518217000,"contributors":[{"name":"weiser","username":"weiser","email":"1134685563@qq.com","commits":1,"url":"https://github.com/weiser"}]},"readingTime":{"minutes":10.48,"words":3143},"filePathRelative":"language/Java/multithreading/lockless/Atomic.md"}');export{r as comp,v as data};
