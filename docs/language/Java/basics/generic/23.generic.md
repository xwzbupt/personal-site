...
上述代码编译失败，编译器提示test(students);这一语句类型不匹配。尽管Student是Member的子类，但是，List<Student>跟List<Member>并没有任何继承关系，所以，将List<Student>类型的数据传递给List<Member>会报错。对于这个问题，我们只需要使用通配符配合extends上界限定符即可解决。
