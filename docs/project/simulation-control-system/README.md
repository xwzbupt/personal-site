...
1. **异构节点如何统一控制**：不同操作系统和软件的控制方式不一致，但上层流程不应感知这些差异。
2. **跨节点流程如何可靠推进**：任务存在明确的前后依赖，不能因为一条消息“发送成功”就直接进入下一阶段。
3. **重复、超时和异常如何处理**：网络链路可能出现丢失、乱序或重复，软件启停又属于有副作用的操作，盲目重试会造成二次执行。
4. **多轮任务如何减少人工值守**：参数准备、阶段推进、数据采集、停止与复位需要形成完整自动化闭环。
5. **控制链路如何不被数据写入拖慢**：运行数据量和存储波动不能影响关键命令与状态反馈。
